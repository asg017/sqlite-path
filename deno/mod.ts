...
let path: string;
try {
  const customPath = Deno.env.get("DENO_SQLITE_PATH_PATH");
  if (customPath) path = customPath;
  else {
    path = await download({
      url: {
        darwin: {
          x86_64: `${BASE}/deno-darwin-x86_64.path0.dylib`,
        },
        windows: {
          x86_64: `${BASE}/deno-windows-x86_64.path0.dll`,
        },
        linux: {
          x86_64: `${BASE}/deno-linux-x86_64.path0.so`,
        },
      },
      suffixes: {
        darwin: "",
  ...
  const error = new Error("Failed to load sqlite-path extension");
  error.cause = e;
...
/**
 * Returns the full path to the compiled sqlite-path extension.
 * Caution: this will not be named "path0.dylib|so|dll", since plug will
...
/**
 * Entrypoint name for the sqlite-path extension.
 */
export const entrypoint = "sqlite3_path_init";
...
/**
 * Loads the sqlite-path extension on the given sqlite3 database.
 */
